...
import threading, time, json, struct
...
class MainFrame(tk.Frame):
    def __init__(self, master, agents, lock):
        ...
        self.lst = tk.Listbox(right, width=24)
        self.lst.pack(padx=6, pady=6)
...
class AgentView(tk.Toplevel):
    def __init__(self, master, agent_id, agents, lock):
        ...
        self.agents = agents
        self.lock = lock
        self.scale = 0.5        # Start zoomed out (~640p if 1080p monitor)
        self.running = True
    ...
    def toggle_control(self):
        if not self.controling:
            ...
            def on_press(key):
                ...
                if kstr is None:
                    kstr = 'Key.' + key.name if hasattr(key, 'name') else str(key)
