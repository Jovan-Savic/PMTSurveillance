...
def start_video_server(host, agents, lock):
    ...
    while True:
        ...
        with lock: agents.setdefault(agent_id, {})['video_conn'] = conn
...
def handle_video(agent_id, conn, agents, lock):
    try:
        while True:
            ...
            with lock:
                if agent_id in agents: agents[agent_id]['last_frame'] = frame
    except Exception as e: print('handle_video error', e)
    finally:
        conn.close()
        with lock: agents.pop(agent_id, None)
...
def start_audio_server(host, agents, lock):
    ...
    while True:
        ...
        if not aid_raw: conn.close(); continue
        agent_id = aid_raw.rstrip(b'\0').decode('utf-8', errors='ignore')
        with lock: agents.setdefault(agent_id, {})['audio_conn'] = conn
...
def handle_audio(agent_id, conn, agents, lock):
    ...
    except Exception as e: print('handle_audio err', e)
    finally:
        conn.close()
        with lock: agents.pop(agent_id, None)
...
def start_control_server(host, agents, lock):
    ...
    while True:
        ...
        if not aid_raw: conn.close(); continue
        agent_id = aid_raw.rstrip(b'\0').decode('utf-8', errors='ignore')
        with lock: agents.setdefault(agent_id, {})['control_conn'] = conn
...
def handle_control_conn(agent_id, conn, agents, lock):
    try:
        ...
        conn.close()
        with lock:
            if agent_id in agents: agents.pop(agent_id, None)